...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] MovementHeadNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is MovementHeadNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MovementHeadNetworkObject.Length > 0 && MovementHeadNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MovementHeadNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<MovementHeadBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateMovementHead instead, its shorter and easier to type out ;)")]
		public MovementHeadBehavior InstantiateMovementHeadNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MovementHeadNetworkObject[index]);
			var netBehavior = go.GetComponent<MovementHeadBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MovementHeadBehavior>().networkObject = (MovementHeadNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of MovementHead
		/// </summary>
		/// <returns>
		/// A local instance of MovementHeadBehavior
		/// </returns>
		/// <param name="index">The index of the MovementHead prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public MovementHeadBehavior InstantiateMovementHead(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MovementHeadNetworkObject[index]);
			var netBehavior = go.GetComponent<MovementHeadBehavior>();
